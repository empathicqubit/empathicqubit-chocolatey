...
var releasesText = HttpGet("https://api.github.com/repos/AppleWin/AppleWin/releases?per_page=1");
...
dynamic asset = assets.First((dynamic x) => new System.Text.RegularExpressions.Regex("^AppleWin.*\\.zip", System.Text.RegularExpressions.RegexOptions.IgnoreCase).IsMatch((string)x.name));

var url = (string)asset.browser_download_url;
var name = (string)asset.name;
var ver = new System.Text.RegularExpressions.Regex("([0-9\\.]+)\\.zip").Match(name).Groups[1].Value;
...
ChocolateyPack("./applewin.nuspec", new ChocolateyPackSettings {
	Version = ver,
});
